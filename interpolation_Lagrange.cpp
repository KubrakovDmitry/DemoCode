...
int main() {
	...
	double Xo = 3*M_PI/16;
	double MX[4];
	double eest;//îöåíêà ïîãðåøíîñòè
	MX[0] = 0;
	MX[1] = M_PI/8;
	MX[2] = M_PI/4;
	MX[3] = 3*M_PI;
	
	cout << " Èíòåðïîëÿöèîííûé ìíîãî÷ëåí Ëàãðàíæà " << endl;
	cout << " Íà÷àëî " << endl;
	double L3; //ìíîãî÷ëåí Ëàãðàíæà
	double MFi[4];//çíà÷åíèÿ ôóíêöèè
	double WSht[4];//àìåãà øòðèõ
	double D[4];//÷àñòíîå ôóíêöèè è àìåãè øòðèõ
	double RL[4];
	void funFi(double MX[4], double MFi[4]);
	void funWSht(double MX[4], double WSht[4]);
	void funFiDivWSht(double WSht[4], double D[4], double MFi[4]); // ÷àñòíàÿ ïðîèçâîäíàÿ
	double Epselan_of_X(double X, double MX[4]);//âû÷èñëåíèå îöåíêè ïîãðåøíîñòè èíòåðïîëÿöèè
	...
	int J = 0;
	if(Result<eest)//ñðàâíåíèå îöåíêè ñ ïîãðåøíîñòüþ
	{
		J=1;
		cout << " J =" << J << endl;
		cout << " Ïîãðåøíîñòü äîïóñêàåòñÿ òî÷íîñòü " <<  endl;
	}
	else{
		cout << " J =" << J << endl;
	}
	cout << " Êîíåö " << endl << endl;
	cout << " Èíòåðïîëÿöèîííûé ìíîãî÷ëåí Íüþòîíà " << endl;
	cout << " Íà÷àëî " << endl;	
	double Fx0x1(double X0, double X1);//ðàçäåë¸ííàÿ ðàçíîñòü 1-ãî ïîðÿäêà
	double Fx0x1x2(double X0, double X1, double X2);//ðàçäåë¸ííàÿ ðàçíîñòü 2-ãî ïîðÿäêà
	double Fx0x1x2X3(double X0, double X1, double X2, double X3);//ðàçäåë¸ííàÿ ðàçíîñòü 3-ãî ïîðÿäêà
...
double Epselan_of_X(double X, double MX[4])//âû÷èñëåíèå îöåíêè ïîãðåøíîñòè èíòåðïîëÿöèè
{
	double dofnl_5 = 0; //ïðîèçâîäíàÿ ïÿòîãî ïîðÿäêà(äëÿ ëîãàðèìà îí ðàâíå 1/pow(MX[i], 5);)
	double k;
	int res=1;
	double amega;
	for(int i= 0; i<4; i++)//ïîèñê ìàêñèìàëüíîãî çíà÷åíèÿ ìîäóëÿ ïðîèçâîäíîé 
	{
		k = 16/pow(cos(MX[i]), 2)+120*pow(sin(MX[i]),2)/pow((cos(MX[i])),6);
		if(dofnl_5 < fabs(k))//åñëè òåêóùàÿ ìàêñ. ïðîèçâ. ìåíüøå íîâîé
		{
			dofnl_5 = fabs(k);
		}
//		cout << " k= " << k << endl;
